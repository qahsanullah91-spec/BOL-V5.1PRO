...
import { getReceivablesAgingReport, canViewFinancials } from "@/lib/reports/management-reporting-service"
...
export async function GET(request: NextRequest) {
  ...
  if (!canViewFinancials(role)) {
    return NextResponse.json({ success: false, error: "Forbidden: Unauthorized to view Receivables reports" }, { status: 403 })
  }
  ...
  try {
    ...
    const report = await getReceivablesAgingReport(filters, role)
    return NextResponse.json({ success: true, report })
  } catch (error: any) {
    console.error("[Receivables API] Error:", error)
    return NextResponse.json({ success: false, error: error.message || "Failed to generate Receivables report" }, { status: 500 })
  }
}
